...
export async function findTeamStandings(teamId: string, season: string = '2025') {
  const leaguesResult = await db.execute(sql`
  ...
  if (!leaguesResult.rows[0]) return null;
  const primaryLeagueId = (leaguesResult.rows[0] as any).id;
  ...
  const standings = await db.execute(sql`
    ...
    WHERE s.league_id = ${primaryLeagueId}
      AND s.season = ${season}
    ORDER BY s.position ASC
  `);

  const homeStats = await db.execute(sql`
    SELECT
      COUNT(*) FILTER (WHERE status='FINISHED') AS played,
      COUNT(*) FILTER (WHERE status='FINISHED' AND home_score > away_score) AS wins,
      COUNT(*) FILTER (WHERE status='FINISHED' AND home_score = away_score) AS draws,
      COUNT(*) FILTER (WHERE status='FINISHED' AND home_score < away_score) AS losses,
    ...
    FROM matches
    WHERE home_team_id = ${teamId} AND season = ${season} AND league_id = ${primaryLeagueId}
  `);

  const awayStats = await db.execute(sql`
    SELECT
      COUNT(*) FILTER (WHERE status='FINISHED') AS played,
      COUNT(*) FILTER (WHERE status='FINISHED' AND away_score > home_score) AS wins,
      COUNT(*) FILTER (WHERE status='FINISHED' AND away_score = home_score) AS draws,
      COUNT(*) FILTER (WHERE status='FINISHED' AND away_score < home_score) AS losses,
    ...
    FROM matches
    WHERE away_team_id = ${teamId} AND season = ${season} AND league_id = ${primaryLeagueId}
  `);

  return {
    leagues: leaguesResult.rows,
    league: leaguesResult.rows[0],
    table: standings.rows,
    homeStats: homeStats.rows[0] ?? null,
    awayStats: awayStats.rows[0] ?? null,
  };
}
