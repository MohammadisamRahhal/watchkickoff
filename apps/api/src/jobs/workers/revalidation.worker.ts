/**
 * revalidation worker — BullMQ job processor.
...
const logger = createLogger('worker:revalidation');

export function createRevalidationWorker(): Worker {
  ...
  worker.on('failed', (job, err) => {
    logger.error({ jobId: job?.id, err }, 'revalidation job failed');
  });

  worker.on('error', (err) => {
    logger.error({ err }, 'revalidation worker error');
  });
