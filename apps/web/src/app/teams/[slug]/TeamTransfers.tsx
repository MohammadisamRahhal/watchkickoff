...
function formatFee(amount: number | null, type: string): string {
  ...
  if (type === 'LOAN' || type === 'LOAN_RETURN' || type === 'LOAN_END') return 'Loan';
  if (!amount) return '—';
...
export default function TeamTransfers({ transfers, teamId }: { transfers: any[]; teamId: string }) {
      ...
      <a key={t.id} href={`/players/${t.player_slug}`} style={{ textDecoration: 'none' }}>
        <div style={{
          ...
          <div style={{
            ...
            background: direction === 'in' ? '#22c55e18' : '#ef444418',
            border: `1px solid ${direction === 'in' ? '#22c55e40' : '#ef444440'}`,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 13, flexShrink: 0, color: direction === 'in' ? '#22c55e' : '#ef4444',
            fontWeight: 800,
          }}>
            {direction === 'in' ? '↓' : '↑'}
          </div>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2, display: 'flex', alignItems: 'center', gap: 5 }}>
              {direction === 'in' ? (
                <>{t.from_crest && <img src={t.from_crest} style={{ width: 12, height: 12, objectFit: 'contain' }} alt="" />}<span>{t.from_team_name ?? 'Unknown'}</span></>
              ) : (
                <>{t.to_crest && <img src={t.to_crest} style={{ width: 12, height: 12, objectFit: 'contain' }} alt="" />}<span>{t.to_team_name ?? 'Unknown'}</span></>
              )}
              {t.transfer_date && <span>• {new Date(t.transfer_date).getFullYear()}</span>}
            </div>
          </div>
          <div style={{ textAlign: 'right', flexShrink: 0 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: t.fee_type === 'FREE' ? 'var(--text-muted)' : '#22c55e' }}>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
      <div>
        <h3 style={{ margin: '0 0 12px', fontSize: 12, fontWeight: 700, color: '#22c55e', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'flex', alignItems: 'center', gap: 6 }}>
          ↓ Arrivals <span style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 400 }}>({incoming.length})</span>
        </h3>
        {renderList(incoming, 'in')}
      </div>
      <div>
        <h3 style={{ margin: '0 0 12px', fontSize: 12, fontWeight: 700, color: '#ef4444', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'flex', alignItems: 'center', gap: 6 }}>
          ↑ Departures <span style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 400 }}>({outgoing.length})</span>
        </h3>
