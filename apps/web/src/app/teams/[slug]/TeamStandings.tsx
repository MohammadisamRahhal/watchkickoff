...
export default function TeamStandings({ standings, teamId, teamSlug }: Props) {
  ...
  return (
    <div>
      {/* Header bar: League name + season dropdown + Overall/Home/Away */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, padding: '10px 16px', background: 'var(--bg-card)', borderRadius: 10, border: '1px solid var(--border)', flexWrap: 'wrap' }}>
        ...
        {leagues.length > 1 ? (
          <select
            ...
            style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', background: 'var(--bg-card)', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-body)', outline: 'none', flex: 1 }}
          >
            {leagues.map((l: any) => (
        ...
        <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
          {(['overall','home','away'] as ViewType[]).map(v => (
            ...
            }}>{v === 'overall' ? 'Overall' : v === 'home' ? 'Home' : 'Away'}</button>
          ))}
        </div>
      </div>
