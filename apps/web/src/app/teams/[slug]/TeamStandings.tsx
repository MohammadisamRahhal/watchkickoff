...
const ZONE_COLOR: Record<string, string> = {
  PROMOTION: '#22c55e', CHAMPIONSHIP: '#3b82f6', RELEGATION: '#ef4444', NONE: 'transparent',
};

export default function TeamStandings({ standings, teamId }: { standings: any; teamId: string }) {
  if (!standings?.table?.length) return (
    <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-muted)' }}>No standings data available.</div>
  );
  ...
  return (
    <div>
        ...
        {table.map((row: any, i: number) => {
          ...
          return (
            <div key={row.team_id} style={{
              display: 'grid',
              gridTemplateColumns: '32px 1fr 36px 36px 36px 36px 36px 36px 44px',
              padding: '9px 14px',
              background: isTeam ? 'rgba(29,78,216,0.06)' : 'transparent',
              borderTop: i > 0 ? '1px solid var(--border-subtle)' : 'none',
              borderLeft: isTeam ? '3px solid var(--blue)' : '3px solid transparent',
              alignItems: 'center',
            }}>
      ...
      <div style={{ display: 'flex', gap: 16, marginTop: 10, flexWrap: 'wrap' }}>
        {[['PROMOTION','#22c55e','Promotion / UCL'],['CHAMPIONSHIP','#3b82f6','Europa League'],['RELEGATION','#ef4444','Relegation']].map(([z,c,l]) => (
