              ...
              {players.map((p: any) => (
                <a key={p.id} href={`/players/${p.slug}`} style={{ textDecoration: 'none' }}>
                  <div style={{ background: 'var(--bg-card)', borderRadius: 8, padding: '10px 12px', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 38, height: 38, borderRadius: '50%', background: POS_COLOR[pos] + '18', border: `2px solid ${POS_COLOR[pos]}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, color: POS_COLOR[pos], fontWeight: 800, flexShrink: 0 }}>
                      {p.name.charAt(0)}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 1, display: 'flex', alignItems: 'center', gap: 5 }}>
                        {p.nationality_code && <img src={`https://flagcdn.com/16x12/${p.nationality_code.toLowerCase()}.png`} style={{ width: 14, height: 11, borderRadius: 1 }} alt="" />}
