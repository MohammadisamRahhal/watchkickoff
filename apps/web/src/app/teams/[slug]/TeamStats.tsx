...
export default function TeamStats({ stats }: { stats: any }) {
  if (!stats) return (
    <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-muted)' }}>No stats available.</div>
  );
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: 10, padding: 18, border: '1px solid var(--border)' }}>
        <h3 style={{ margin: '0 0 14px', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Season 2025/26</h3>
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: 10, padding: 18, border: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
          {[
            { label: 'Scored', value: gf, color: '#22c55e' },
            { label: 'Conceded', value: ga, color: '#ef4444' },
            { label: 'GD', value: `${gf-ga>=0?'+':''}${gf-ga}`, color: 'var(--blue)' },
          ].map(s => (
            <div key={s.label} style={{ flex: 1, textAlign: 'center', background: 'var(--bg-elevated)', borderRadius: 8, padding: '12px 6px', border: '1px solid var(--border)' }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
          Avg scored: <strong style={{ color: 'var(--text)' }}>{played > 0 ? (gf/played).toFixed(1) : '0.0'}</strong> per game
        </div>
        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>
          Avg conceded: <strong style={{ color: 'var(--text)' }}>{played > 0 ? (ga/played).toFixed(1) : '0.0'}</strong> per game
        </div>
      </div>

      <div style={{ background: 'var(--bg-card)', borderRadius: 10, padding: 18, border: '1px solid var(--border)', gridColumn: 'span 2' }}>
        <h3 style={{ margin: '0 0 14px', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Home vs Away</h3>
        ...
        <div style={{ marginTop: 14, padding: '10px 14px', background: 'var(--bg-elevated)', borderRadius: 8, border: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 800, color: '#22c55e' }}>{winRate}%</span>
          </div>
          <div style={{ height: 6, background: 'var(--border)', borderRadius: 3, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${winRate}%`, background: '#22c55e', borderRadius: 3 }} />
          </div>
        </div>
      </div>
    </div>
