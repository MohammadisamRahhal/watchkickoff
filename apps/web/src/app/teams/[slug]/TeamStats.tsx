  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: 10, padding: 18, border: '1px solid var(--border)' }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Avg scored: <strong style={{ color: 'var(--text)' }}>{played > 0 ? (gf/played).toFixed(1) : '0.0'}</strong> per game</div>
        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>Avg conceded: <strong style={{ color: 'var(--text)' }}>{played > 0 ? (ga/played).toFixed(1) : '0.0'}</strong> per game</div>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {[{label:'🏠 Home',p:hp,w:hw},{label:'✈️ Away',p:ap,w:aw}].map(({label,p,w}) => (
            <div key={label} style={{ background: 'var(--bg-elevated)', borderRadius: 8, padding: 14, textAlign: 'center', border: '1px solid var(--border)' }}>
              ...
              <div style={{ marginTop: 10, fontSize: 12, color: 'var(--text-muted)' }}>Win rate: <strong style={{ color: '#22c55e' }}>{p > 0 ? Math.round((w/p)*100) : 0}%</strong></div>
            </div>
          ))}
        </div>
