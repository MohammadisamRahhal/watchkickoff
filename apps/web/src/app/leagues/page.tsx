...
export default async function LeaguesPage() {
  ...
  return (
    <div className="container" style={{ paddingTop: 28, paddingBottom: 48 }}>
      <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 28, fontWeight: 700, letterSpacing: '0.03em', marginBottom: 24 }}>
        Leagues
      </h1>
      <div style={{ display: 'grid', gap: 8 }}>
        {Array.from(byCountry.entries()).map(([countryCode, countryLeagues]) => (
          <div key={countryCode}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0 6px', borderBottom: '1px solid var(--border)', marginBottom: 4 }}>
            ...
            <div style={{ display: 'grid', gap: 2, marginBottom: 16 }}>
              {countryLeagues.map((league) => (
                <a key={league.id} href={'/leagues/' + league.slug} className="league-row">
                  <div>
                    <div style={{ fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 600 }}>{league.name}</div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 1 }}>{league.season} {league.type}</div>
                  </div>
